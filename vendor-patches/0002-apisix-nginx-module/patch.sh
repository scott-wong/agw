#!/usr/bin/env bash
# Vendored from api7/apisix-nginx-module（补丁来源：上游 master + PR #125，
# 使 patch.sh 识别 openresty-1.31.1.*）。回退条件：上游合入 PR #125 并发布
# 含该修复的正式 release 后，删除本覆盖、回归纯上游脚本。
# 序号：0002（应用顺序见 scripts/build-apisix-runtime.sh）
set -euo pipefail
